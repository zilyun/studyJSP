package ex8;
/*
 * 자바빈(JavaBean) 자바빈 규약에 따라 작성된 자바 클래스입니다.
 * 예를 들어 회원 정보, 게시판 글의 정보를 출력할 때 정보를 저장하고 있는 자바빈 객체를 사용하게 됩니다.
 * 
 * * 다음은 자바빈의 규약입니다.
 ...
 * 	  단, 읽기 전용인 경우 get 또는 is 메서드만 저의할 수 있다.
 ...
 * 
 * * 메서드 이름을 사용해서 프로퍼티(prperty)의 이름을 결정하게 됩니다.
 *   예를 들어 프로퍼티의 이름이 name이고 값의 타입이 int인 경우 프로퍼티와 관련된 메서드의 이름은 다음과 같이 결정됩니다.
 *   public void setName(int value){};
 *   public int getName(){};
 * 	
 * 	 setName()과 getName()를 통해서 관리되는 데이터를 프로퍼티(property, 성질)라고 부릅니다. 
 *   
 ...
 *   여러 개의 정보들을 데이터 빈이라는 하나의 객체에 저장하게 되고 이 객체를 전달하면 각 정보를 하나씩 전달할 필요가 없으며 
 *   한꺼번에 모든 정보가 전달됩니다.
