...
/*
 * 1. @은 annotation으로 자바 주석문처럼 소스 안에 @기호와 함께 사용됩니다.
 * 
 * 2. 주석문처럼 컴파일러에 정보를 알려주는 기능 또는 자바 프로그램 실행에 관한 내용을 
 * 		설정하는 용도로 사용됩니다.
 * 
 * 3. @WebServlet 설정을 통한 서블릿 접근 방법은 서블릿 3.0부터 지원합니다.
 * 		더 이상 web.xml 에 서블릿 매핑에 관한 설정을 할 필요가 없습니다.
 * 
 * 4. @WebServlet 어노테이션은 urlPatterns 속성을 값으로 찾는데 이 속성은
...
public class LifeCycle설명 extends HttpServlet { // http://localhost:8080/JSP/LifeCycle
/*
 * 1. serialVersionUID는 같은 클래스임을 알려주는 식별자 역할을 하며 
 * 	  Serialzable 인터페이스를 구현한 클래스를 컴파일하면 자동적으로 
 * 	  serialVersionUID 정적 필드가 추가됩니다.
 * 
 * 2. serialVersionUID는 자바의 직렬화 (객체의 데이터를 byte 형태의 데이터로 변환하는 기술)와
 * 	  역직렬화(byte로 변환된 데이터를 원래의 객체 데이터로 변환하는 기술) 실행될 때
 * 	  자동으로 부여됩니다.
 * 
 * 3. 역직렬화 시 직렬화 가능 클래스의 내용이 그대로이면 같은 번호가 부여되고 
