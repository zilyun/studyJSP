<%--
		톰캣에 설정할 경우 전체, 프로젝트에 xml을 설정할 경우 프로젝트만 적용된다.

		1. Java Naming and Directory Interface (JNDI)
			* Java 애플리케이션이 네트워크에서 이름(name)을 이용하여 객체를 찾고 가져올 수 있도록 하는 표준 API 입니다.
			...
			* 이 때 JNDI의 기본 이름 중 java:comp/env/jdbc는 JDBC Data Source의 이름입니다.
		...
		2. DBCP(Database Connection Pool)
			  ...
			  웹 프로그램에서는 데이터베이스의 환경설정과 연결 관리 등을 따로 XML 파일이나 
			  속성 파일을 사용해서 관리하고, 
			  이렇게 설정된 정보는 이름을 사용하여 획득하는 방법을 사용합니다.
			  
		  * DBCP를 이용한 커넥션 풀을 사용하기 위해 먼저 톰캣에 DataSource를 등록해야 합니다.
